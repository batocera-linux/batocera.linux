...
test -n "${updateurl}" && batoceraupdateurl="${updateurl}"
...
do_listArchs() {
	wget -qO - "${batoceraupdateurl}/installs.txt" | sed -e s+'^\([^/]*\)/.*$'+'\1'+ | sort -u
}
...
do_install() {
	...
	# url
	RELATIVPATH=$(wget -qO - "${batoceraupdateurl}/installs.txt" | grep -E "^${INSARCH}/" | head -1)
	FILEBASENAME=$(basename "${RELATIVPATH}")
	INSIMG="${IMGFOLDER}/${INSARCH}/${FILEBASENAME}"
	test -z "${RELATIVPATH}" && return 1
	url="${batoceraupdateurl}/${RELATIVPATH}"

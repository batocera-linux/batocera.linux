...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-vim1s"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-vim1s/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-vim1s/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/kvim1s.dtb"  "${BATOCERA_BINARIES_DIR}/boot/boot/"          || exit 1

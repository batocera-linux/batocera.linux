...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-odroidn2l"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-odroidn2l/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-odroidn2l/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-g12b-odroid-n2l.dtb"     "${BATOCERA_BINARIES_DIR}/boot/boot/meson-g12b-odroid-n2l.dtb"     || exit 1

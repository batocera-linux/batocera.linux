...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-vim3l"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-vim3l/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-vim3l/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1
...
cp "${BINARIES_DIR}/meson-sm1-khadas-vim3l.dtb" "${BATOCERA_BINARIES_DIR}/boot/boot/"     || exit 1
...
cp "${BATOCERA_BINARIES_DIR}/uboot-vim3l/u-boot.raw" "${BATOCERA_BINARIES_DIR}/boot/u-boot.bin" || exit 1

...
mkdir -p "${BATOCERA_BINARIES_DIR}/build-uboot-odroidc2"     || exit 1
cp "${BOARD_DIR}/build-uboot.sh"          "${BATOCERA_BINARIES_DIR}/build-uboot-odroidc2/" || exit 1
cd "${BATOCERA_BINARIES_DIR}/build-uboot-odroidc2/" && ./build-uboot.sh "${HOST_DIR}" "${BOARD_DIR}" "${BINARIES_DIR}" || exit 1

...
rsync -rltXv --exclude-from=/recalbox/scripts/rsync-update/always-exclude.cfg --exclude-from=/recalbox/scripts/rsync-update/exclude-share.cfg rsync://${RSYNC_USER}@${RSYNC_SERVER}/recalbox-share-${version} /recalbox/share >> /root/updates.log
if (($? != 0)); then
        echo "error : unable to update share" >> /root/updates.log
        exit 91
fi
echo "info : starting boot update" >> /root/updates.log
rsync -rltXv --exclude-from=/recalbox/scripts/rsync-update/always-exclude.cfg --exclude-from=/recalbox/scripts/rsync-update/exclude-boot.cfg rsync://${RSYNC_USER}@${RSYNC_SERVER}/recalbox-boot-${version} /boot >> /root/updates.log
if (($? != 0)); then
        echo "error : unable to update boot" >> /root/updates.log
        exit 91
fi
echo "info : starting root update" >> /root/updates.log
rsync -aXv --exclude-from=/recalbox/scripts/rsync-update/always-exclude.cfg --exclude-from=/recalbox/scripts/rsync-update/exclude-root.cfg rsync://${RSYNC_USER}@${RSYNC_SERVER}/recalbox-root-${version} / >> /root/updates.log
if (($? != 0)); then

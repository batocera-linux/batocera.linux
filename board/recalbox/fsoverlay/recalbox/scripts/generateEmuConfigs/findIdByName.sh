function findIdByName {
	...
	regex="js([0-9]+)"
	echo "searching id of joystick '$name' in $handlers"
	IFS=$'\n'
	for handler in $handlers; do
		echo "searching in $handler"
	    	[[ $handler =~ $regex ]]
		joystickIndex="${BASH_REMATCH[1]}"
		echo "values ${BASH_REMATCH['1']} ${usedJoysticks[$joystickIndex]}"

		if [ -n "$joystickIndex" ] && [ -z "${usedJoysticks[$joystickIndex]}" ]; then
			echo "found joystick index for $1 : $joystickIndex"

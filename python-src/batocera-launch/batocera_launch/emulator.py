...
from batocera_launch.draw.gun_help import generate_gun_help
...
from batocera_launch.fs import mount_squashfs
from batocera_launch.functools import cached_property
from batocera_launch.paths import ES_GUNS_ART_METADATA, SYSTEM_DECORATIONS, USER_DECORATIONS
...
class Emulator(ABC):
    ...
    def guns_art_metadata(self) -> Mapping[str, str]:
        ...
        if ES_GUNS_ART_METADATA.exists():
            values = get_games_meta_data(ES_GUNS_ART_METADATA, self.system, self.rom)
        else:
    ...
    def prepare_hud_bezel(self) -> Path | None:
    ...
    async def prepare_hud(self, command: Command, /) -> None:
        hud_bezel = self.prepare_hud_bezel()
    ...
    def prepare_gun_help(self) -> None:
        if not (self.config.use_guns and self.guns):
            _logger.info('skipping gun help generation')
            return

        try:
            generate_gun_help(self.guns, self.guns_art_metadata, self.resolution)
        except Exception:
    ...
    async def prepare_rom(emulator: Emulator, /) -> AsyncGenerator[Path]:
        ...
        async with mount_squashfs(
            emulator.rom,
            writable_dir=(SAVES / emulator.rom.parent.name / emulator.rom.stem) if emulator.needs_overlayfs else None,
        ) as squashfs_rom:
            yield squashfs_rom

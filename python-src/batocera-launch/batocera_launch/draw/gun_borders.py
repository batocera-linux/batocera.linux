...
from batocera_launch.devices.x11 import Display, ShapeKind, WindowAttributes, XSetWindowAttributes
...
def _create_click_through_rectangle(display: Display, x: int, y: int, width: int, height: int, color: int, /) -> None:
    ...
    window = display.create_window(
        ...
        width,
        height,
        attributes_mask=WindowAttributes.back_pixel | WindowAttributes.ovrride_redirect,
        attributes=attributes,
    )
    empty_region = display.create_empty_region()
    window.set_window_shape_region(ShapeKind.input, 0, 0, empty_region)
    display.destroy_region(empty_region)
    window.map_raised()


def _draw_border(
    display: Display, offset_x: int, offset_y: int, width: int, height: int, thickness: int, color: int, /
) -> None:
    if thickness <= 0:
        return

    _create_click_through_rectangle(display, offset_x, offset_y, width, thickness, color)
    _create_click_through_rectangle(display, offset_x, offset_y + height - thickness, width, thickness, color)
    _create_click_through_rectangle(display, offset_x, offset_y + thickness, thickness, height - 2 * thickness, color)
    _create_click_through_rectangle(
        display,
...
def draw_gun_borders(border_size: str, border_color: str, border_ratio: str | None, /) -> None:
    from batocera_launch.devices.x11 import open_display
    ...
    with open_display() as display:
        ...
        # outer
        _draw_border(display, region_offset_x, 0, region_width, screen_height, outer_thickness, 0x000000)

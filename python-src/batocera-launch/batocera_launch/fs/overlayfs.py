...
async def _unmount_and_remove(mount_point: Path, /) -> None:
    if mount_point.is_mount():
        ...
        if result.returncode:
            _logger.error(
                "failed unmounting '%s' (rc=%d) because %s", mount_point, result.returncode, result.stderr.strip()
            )
...
async def _mount(read_only_dir: Path, writable_upper_dir: Path, writable_work_dir: Path, mount_point: Path, /) -> bool:
    ...
    result = await run('mount', '-t', 'overlay', 'overlay', '-o', components, mount_point, text=True)

    if not result.returncode:
        _logger.debug("mounted '%s' with components '%s'", mount_point, components)
    else:
        _logger.error(
            "failed mounting '%s' with components '%s' (rc=%d) because %s",
            mount_point,
            components,
            result.returncode,
            result.stderr.strip(),

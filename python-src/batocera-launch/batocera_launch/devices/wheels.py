...
from batocera_launch.devices.device import DeviceInfoDict, get_device_info, input_event_path_to_id
...
if TYPE_CHECKING:
    ...
    from batocera_launch.devices.controller import Controller, ControllerList
...
async def configure_wheels(
    system_name: str, system_config: SystemConfig, controllers: ControllerList, metadata: dict[str, str], /
        ...
        if controller.device_path in devices and devices[controller.device_path].is_wheel:
            ...
            for md_key, md_value in wheel_metadata.items():
                if (
                    ...
                    and md_value in _SYSTEM_MAPPING.get(system_name, {})
            ...
            for md_key, md_value in wheel_metadata.items():
                if (wheel_key := _WHEEL_MAPPING.get(md_key)) is not None and (
                    wanted_key := _SYSTEM_MAPPING.get(system_name, {}).get(md_value)
                ) is not None:

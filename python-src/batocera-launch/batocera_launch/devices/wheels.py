...
from .device import DeviceInfoDict, get_device_info, input_event_path_to_id
...
@asynccontextmanager
async def configure_wheels(
    system_config: SystemConfig, controllers: ControllerList, metadata: dict[str, str], /
) -> AsyncGenerator[tuple[ControllerList, DeviceInfoDict]]:
    if not system_config.use_wheels or not controllers:
    ...
    devices = await asyncio.to_thread(get_device_info)
    ...
    for controller in controllers:
        if controller.device_path in devices and devices[controller.device_path].is_wheel:
            ...
            for md_key, md_value in wheel_metadata.items():
                if (
                    ...
                    and md_value in _SYSTEM_MAPPING.get(system_config.system, {})
            ...
            for md_key, md_value in wheel_metadata.items():
                if (wheel_key := _WHEEL_MAPPING.get(md_key)) is not None and (
                    wanted_key := _SYSTEM_MAPPING.get(system_config.system, {}).get(md_value)
                ) is not None:
    ...
    saved_ranges: list[tuple[Path, str]] = []
    ...
    for controller in controllers:
        if (
            ...
            and device.is_wheel
            and device.wheel_rotation is not None
        ):
            ra = device.wheel_rotation
            wanted_ra = ra
            wanted_deadzone = 0
            wanted_midzone = 0
            ...
            if 'wheel_rotation' in system_config:
                wanted_ra = int(system_config['wheel_rotation'])
            if 'wheel_deadzone' in system_config:
                wanted_deadzone = int(system_config['wheel_deadzone'])
            if 'wheel_midzone' in system_config:
                wanted_midzone = int(system_config['wheel_midzone'])

            _logger.info(
                'wheel rotation angle is %s ; wanted wheel rotation angle is %s ; wanted deadzone is %s ; wanted midzone is %s',
                ra,
                wanted_ra,
                wanted_deadzone,
                wanted_midzone,
            ...
            if (range_path := _find_range_path(device.sysfs_path)) is not None:
                saved_ranges.append((range_path, str(ra)))
            ...
            if wanted_ra < ra or wanted_deadzone > 0:
                reconfigure_result = await _reconfigure_angle_rotation(
                    controller, ra, wanted_ra, wanted_deadzone, wanted_midzone
                )
                if reconfigure_result is not None:
                    ...
                    _logger.info(
                        'replacing device %s by device %s for player %s',
                        controller.device_path,
                        newdev,
                        controller.player_number,
                    )
                    devices[newdev] = replace(device, event_id=cast('int', input_event_path_to_id(newdev)))
    ...
    if recompute_sdl_ids:
        ...
        joysticks: dict[int, str] = {device.event_id: node for node, device in devices.items() if device.is_joystick}
    ...
    for controller in controllers:
        if (
            controller.device_path in devices and devices[controller.device_path].is_wheel
    ...
    try:
        yield controllers_new, {key: device for key, device in devices.items() if device.is_wheel}
    finally:
        try:
            await _reset_controllers(procs)
        except Exception:
            _logger.error('hum, unable to reset wheel controllers !')
            # don't fail
        _restore_wheel_ranges(saved_ranges)


def _restore_wheel_ranges(saved_ranges: Iterable[tuple[Path, str]], /) -> None:
    for range_path, original in saved_ranges:
        try:
            range_path.write_text(original)
            _logger.info('wheel range restored to %s through %s', original, range_path)
        except OSError:
            _logger.error('unable to restore the range of %s', range_path)
...
) -> tuple[str, asyncio.subprocess.Process] | None:
    import evdev
...
async def _reset_controllers(wheel_processes: Iterable[asyncio.subprocess.Process]) -> None:

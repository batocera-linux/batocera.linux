...
class Armsx2(Emulator):
    ...
    def config_dir(self) -> Path:
        # the folder is upper-case on disk, unlike the package name
        return CONFIGS / 'ARMSX2'
    ...
    def in_game_ratio(self) -> float:
        config_ratio = self.config.get('armsx2_ratio')
    ...
    def _wheel_type(self) -> str:
        ...
        if config_wheel_type := self.config.get('armsx2_wheel_type'):
    ...
    async def configure(self) -> Command:
        ...
        command_array: list[str | Path] = (
            ['/usr/armsx2/bin/armsx2-qt']
            if configure_emulator(self.rom)
            else ['/usr/armsx2/bin/armsx2-qt', '-nogui', self.rom]
        )
    ...
    async def _configure_ini(self) -> None:
        ...
        pcsx2_ini_config.set('Folders', 'Savestates', '../../../saves/ps2/armsx2/sstates')
        ...
        pcsx2_ini_config.set(
            'EmuCore', 'EnableFastBoot', self.config.get_bool('armsx2_fastboot', True, return_values=('false', 'true'))
        )

        # Cheats
        pcsx2_ini_config.set('EmuCore', 'EnableCheats', self.config.get('armsx2_cheats', 'false'))

        # Widescreen Patches
        pcsx2_ini_config.set(
            'EmuCore', 'EnableWideScreenPatches', self.config.get('armsx2_EnableWideScreenPatches', 'false')
        )

        # No-interlacing Patches
        pcsx2_ini_config.set(
            'EmuCore', 'EnableNoInterlacingPatches', self.config.get('armsx2_interlacing_patches', 'false')
        )
        ...
        if vulkan_info := await get_vulkan_info():
            ...
            if gfxbackend := self.config.get('armsx2_gfxbackend'):
                if gfxbackend == '12':
        ...
        # Ratio
        pcsx2_ini_config.set('EmuCore/GS', 'AspectRatio', self.config.get('armsx2_ratio', 'Auto 4:3/3:2'))

        # Vsync
        pcsx2_ini_config.set('EmuCore/GS', 'VsyncEnable', self.config.get('armsx2_vsync', '0'))

        # Resolution
        pcsx2_ini_config.set('EmuCore/GS', 'upscale_multiplier', self.config.get('armsx2_resolution', '1'))

        # FXAA
        pcsx2_ini_config.set('EmuCore/GS', 'fxaa', self.config.get('armsx2_fxaa', 'false'))

        # FMV Ratio
        pcsx2_ini_config.set('EmuCore/GS', 'FMVAspectRatioSwitch', self.config.get('armsx2_fmv_ratio', 'Auto 4:3/3:2'))
        ...
        pcsx2_ini_config.set(
            'EmuCore/GS', 'hw_mipmap', 'false' if self.config.get('armsx2_mipmapping') == '0' else 'true'
        )

        # Trilinear Filtering
        pcsx2_ini_config.set('EmuCore/GS', 'TriFilter', self.config.get('armsx2_trilinear_filtering', '-1'))

        # Anisotropic Filtering
        pcsx2_ini_config.set('EmuCore/GS', 'MaxAnisotropy', self.config.get('armsx2_anisotropic_filtering', '0'))

        # Dithering
        pcsx2_ini_config.set('EmuCore/GS', 'dithering_ps2', self.config.get('armsx2_dithering', '2'))

        # Texture Preloading
        pcsx2_ini_config.set('EmuCore/GS', 'texture_preloading', self.config.get('armsx2_texture_loading', '2'))

        # Deinterlacing
        pcsx2_ini_config.set('EmuCore/GS', 'deinterlace_mode', self.config.get('armsx2_deinterlacing', '0'))

        # Anti-Blur
        pcsx2_ini_config.set('EmuCore/GS', 'pcrtc_antiblur', self.config.get('armsx2_blur', 'true'))

        # Integer Scaling
        pcsx2_ini_config.set('EmuCore/GS', 'IntegerScaling', self.config.get('armsx2_scaling', 'false'))

        # Blending Accuracy
        pcsx2_ini_config.set('EmuCore/GS', 'accurate_blending_unit', self.config.get('armsx2_blending', '1'))

        # Texture Filtering
        pcsx2_ini_config.set('EmuCore/GS', 'filter', self.config.get('armsx2_texture_filtering', '2'))

        # Bilinear Filtering
        pcsx2_ini_config.set('EmuCore/GS', 'linear_present_mode', self.config.get('armsx2_bilinear_filtering', '1'))

        # Load Texture Replacements
        pcsx2_ini_config.set(
            'EmuCore/GS', 'LoadTextureReplacements', self.config.get('armsx2_texture_replacements', 'false')
        )
        ...
        osd_enabled = self.config.get('armsx2_osd_messages', 'true')

        # OSD Messages Position
        pcsx2_ini_config.set(
            'EmuCore/GS',
            'OsdMessagesPos',
            '0' if osd_enabled == 'false' else self.config.get('armsx2_osd_messages_position', '2'),
        )

        # OSD Performance Position
        pcsx2_ini_config.set('EmuCore/GS', 'OsdPerformancePos', self.config.get('armsx2_osd_performance_position', '0'))

        # Crop Overscan
        crop_overscan = '3' if self.config.get_bool('armsx2_overscan') else '0'
        ...
        # TV Shader
        pcsx2_ini_config.set('EmuCore', 'TVShader', self.config.get('armsx2_shaderset', '0'))
        ...
        # VU thread speedhack
        pcsx2_ini_config.set(
            'EmuCore/Speedhacks', 'vuThread', self.config.get_bool('armsx2_vuthread', return_values=('true', 'false'))
        )

        # EE Cycle Rate speedhack
        pcsx2_ini_config.set('EmuCore/Speedhacks', 'EECycleRate', self.config.get('armsx2_eecyclerate', '0'))
        ...
        if pcsx2_ini_config.has_section('USB1'):
            if self.config.get('armsx2_crosshairs') == '1':
                pcsx2_ini_config.set(
                    'USB1', 'guncon2_cursor_path', str(_ARMSX2_RESOURCES_DIR / 'crosshairs' / 'default.png')
                )
        ...
        if pcsx2_ini_config.has_section('USB2'):
            if self.config.get('armsx2_crosshairs') == '1':
                pcsx2_ini_config.set(
                    'USB2', 'guncon2_cursor_path', str(_ARMSX2_RESOURCES_DIR / 'crosshairs' / 'default.png')
                )
        ...
        if self.config.get('armsx2_crisis_fog') == 'true':
        ...
        multitap_config = self.config.get('armsx2_multitap')

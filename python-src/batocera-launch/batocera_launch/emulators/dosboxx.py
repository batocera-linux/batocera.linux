...
class DosBoxx(Emulator):
    ...
    async def configure(self) -> Command:
        ...
        command: list[str | Path] = ['/usr/bin/dosbox-x', '-exit']
        ...
        if autoexec_file.exists():
            ...
            # in dosbox.auto
            command.extend(['-defaultdir', str(self.rom)])
        else:
        ...
        command.extend(['-fastbioslogo', '-conf', str(custom_config_file)])

        return Command(command, env={'XDG_CONFIG_HOME': CONFIGS})

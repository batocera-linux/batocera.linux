...
if TYPE_CHECKING:
    ...
    from batocera_launch import Controllers, SystemConfig
...
def generate_controllers_config(
    config_dir: Path, config: SystemConfig, controllers: Controllers, /, *, keyboard: bool = False
) -> None:
    input_dir = config_dir / 'input_configs' / 'global'
    input_dir.mkdir(parents=True, exist_ok=True)

    # Lightgun arcade titles are driven through the keyboard (two players on one keyboard).
    if keyboard:
        (input_dir / 'Default.yml').write_text(_ARCADE_KEYBOARD_CONFIG)
        return
    ...
    safe_dump_yaml12(controllers_config, input_dir / 'Default.yml')

...
def get_mapping_full_path(device: evdev.InputDevice[str]) -> Path | None:
    fullpath = None
...
def get_mapping(device: evdev.InputDevice[str] | None) -> dict[int, str]:
    if device is None:
        fullpath = None
    else:
...
class Daemon:
    ...
    input_devices_by_fd: dict[int, evdev.InputDevice[str]] = field(
        init=False, default_factory=dict[int, evdev.InputDevice[str]]
    )

...
def main(args, maxnbplayers):
    ...
    try:
        ...
        # run the emulator
        try:
            evmapy_start(systemName, system.config['emulator'], effectiveCore, effectiveRom, playersControllers)
...
def evmapy_start(system, emulator, core, rom, playersControllers):
    if evmapy_prepare(system, emulator, core, rom, playersControllers):
        subprocess.call(["batocera-evmapy", "start"])

def evmapy_stop():
    subprocess.call(["batocera-evmapy", "stop"])

def evmapy_prepare(system, emulator, core, rom, playersControllers):
                ...
                if "actions_player"+str(nplayer) in padActionConfig:
                    ...
                    for action in padActionsDefined:
                        if "trigger" in action:
                            trigger = evmapy_trigger_mapper(action["trigger"])
...
def evmapy_trigger_mapper(trigger):
    if isinstance(trigger, list):
        new_trigger = []
        for x in trigger:
            new_trigger.append(evmapy_trigger_mapper_string(x))
        return new_trigger
    return evmapy_trigger_mapper_string(trigger)

def evmapy_trigger_mapper_string(trigger):

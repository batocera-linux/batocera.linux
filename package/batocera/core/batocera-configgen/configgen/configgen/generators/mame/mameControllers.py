...
from .mameCommon import get_all_mame_control_mappings, get_mess_system_controls
...
def generatePadsConfig(cfgPath: Path, playersControllers: Controllers, sysName: str, altButtons: MameControlScheme, customCfg: bool, specialController: str, decorations: str | None, useGuns: bool, guns: Guns, useWheels: bool, wheels: DeviceInfoMapping, useMouse: bool, multiMouse: bool, system: Emulator) -> None:
    ...
    mappings, gunmappings, mousemappings = get_all_mame_control_mappings(altButtons, useGuns, useMouse)
    ...
    messControlDict = get_mess_system_controls(sysName, useControls)
                ...
                if nplayer == thisControl.player and xml_input_alt is not None and config_alt is not None:
                    if thisControl.type == 'special':
                        ...
                        xml_input_alt.appendChild(generateAnalogPortElement(pad, config_alt, thisControl.tag, nplayer, pad.index, thisControl.key, mappings_use[thisControl.incMapping], \
                            mappings_use[thisControl.decMapping], pad.inputs[mappings_use[thisControl.incUseMapping]], pad.inputs[mappings_use[thisControl.decUseMapping]], thisControl.reversed, \
...
def generateSpecialPortElement(pad: Controller, config: minidom.Document, tag: str, nplayer: int, padindex: int, mapping: str, key: str, input: Input, reversed: bool, mask: int, default: int, pedalkey: str | None):
    # Special button input (ie mouse button to gamepad)
    xml_port = config.createElement("port")
    xml_port.setAttribute("tag", tag)
...
def generateComboPortElement(pad: Controller, config: minidom.Document, tag: str, padindex: int, mapping: str, kbkey: str, key: str, input: Input, reversed: bool, mask: int | None = None, default: int | None = None):
...
def generateAnalogPortElement(pad: Controller, config: minidom.Document, tag: str, nplayer: int, padindex: int, mapping: str, inckey: str, deckey: str, mappedinput: Input, mappedinput2: Input, reversed: bool, mask: int, default: int, delta: int, axis: str = ''):

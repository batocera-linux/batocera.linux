...
from .mameCommon import get_autorun_command, get_mame_control_scheme, get_mess_system_info
...
class MameGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        if mess_system is None:
            ...
            if customCfg:
                cfgPath = MAME_CONFIG / mess_system.name / "custom"
            else:
                cfgPath = MAME_CONFIG / mess_system.name
            mkdir_if_not_exists(MAME_CONFIG / mess_system.name)
        ...
        if system.config.get_bool("pergamecfg") and mess_system is not None and mess_system.name:
            base_path = MAME_CONFIG / mess_system.name
        ...
        # Mouse
        useMouse = False
        if system.config.get_bool('use_mouse') or (mess_system is not None and mess_system.name):
            useMouse = True
        ...
        if mess_system is None or not mess_system.name:
            ...
            if softList == "":
                ...
                if system.name != "macintosh":
                    if altromtype:
                        ...
                        commandArray += [ f'-{mess_system.rom_type}' ]
                else:
                    if boot_disk:
            ...
            autoRunCmd, autoRunDelay = get_autorun_command(
                system.name, mess_system.auto_run, rom, altromtype or None, softList
            )
            if autoRunCmd:
        ...
        try:
            messSys = mess_system.name if mess_system is not None else ""
            MameGenerator.writeBezelConfig(bezelSet, system, rom, messSys, gameResolution, system.guns_borders_size_name(guns), system.guns_border_ratio_type(guns))
        except Exception:
        ...
        buttonLayout = get_mame_control_scheme(system, rom)

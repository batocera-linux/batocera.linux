...
class WineGenerator(Generator):
    """
    Runs the windows systems, in the shapes batocera-wine established for them:
    ...
    def writableRomDir(self, system, rom: Path) -> Path:
        ...
        return wine.bottle_dir_for(_WINDOWS, wine.runner_base(_runner_name(system.config))[0], rom)
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        if system.name not in (_WINDOWS, _WINDOWS_INSTALLERS):
            raise BatoceraException(f"Invalid system: {system.name}")
    ...
    def __prepare(self, config: SystemConfig, rom: Path, /, *, prefix: Path | None = None) -> wine.Runner:
        ...
        called first, so this is done once and kept.
        """
        if self.__runner is not None:
            return self.__runner

        runner_name = _runner_name(config)

        if prefix is None:
            prefix = wine.bottle_dir_for(_WINDOWS, wine.runner_base(runner_name)[0], rom)

        runner = wine.Runner.for_prefix(
            runner_name, prefix, arch=wine.wanted_arch(prefix, runner_name, win32=config.get_bool('enable_win32')),
        )
    ...
    def __rom_dir(self, config: SystemConfig, rom: Path, /) -> Path:
        """
        Where the autorun.cmd of the game is: a .pc rom keeps the game files next to
        its own, everything else describes itself in its prefix - which for a .wine rom
        or a mounted .wsquashfs is the rom itself.
        """
        runner = self.__prepare(config, rom)

        return rom if rom.is_dir() and runner.bottle_dir != rom else runner.bottle_dir
    ...
    def __setup_prefix(self, config: SystemConfig, runner: wine.Runner, /) -> None:
        ...
        # what shows around the window of a virtual desktop, black rather than wine's blue
        if config.get_bool('virtual_desktop'):

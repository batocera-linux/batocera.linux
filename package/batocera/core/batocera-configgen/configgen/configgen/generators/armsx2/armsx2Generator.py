...
_ARMSX2_BIN_DIR: Final = Path("/usr/armsx2/bin")
_ARMSX2_RESOURCES_DIR: Final = _ARMSX2_BIN_DIR / "resources"
_ARMSX2_CONFIG: Final = CONFIGS / "ARMSX2"
_ARMSX2_BIOS: Final = BIOS / "ps2"

class Armsx2Generator(Generator):
    ...
    def getWheelType(metadata: Mapping[str, str], playingWithWheel: bool, config: SystemConfig):
        ...
        if config_wheel_type := config.get("armsx2_wheel_type"):
            wheel_type = config_wheel_type
        if wheel_type not in Armsx2Generator.wheelTypeMapping:
            wheel_type = "Virtual"
        return wheel_type

    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        pcsx2Patches = _ARMSX2_BIOS / "patches.zip"

        # Remove older config files if present
        inisDir = _ARMSX2_CONFIG / "inis"
        ...
        playingWithWheel = Armsx2Generator.isPlayingWithWheel(system, wheels)

        # Config files
        configureReg(_ARMSX2_CONFIG)
        configureINI(_ARMSX2_CONFIG, _ARMSX2_BIOS, system, rom, playersControllers, metadata, guns, wheels, playingWithWheel)
        configureAudio(_ARMSX2_CONFIG)

        # write our own game_controller_db.txt file before launching the game
        dbfile = _ARMSX2_CONFIG / "game_controller_db.txt"
        write_sdl_controller_db(playersControllers, dbfile)

        commandArray = ["/usr/armsx2/bin/armsx2-qt"] if configure_emulator(rom) else \
              ["/usr/armsx2/bin/armsx2-qt", "-nogui", rom]
        ...
        # wheel metadata
        if not Armsx2Generator.useEmulatorWheels(playingWithWheel, Armsx2Generator.getWheelType(metadata, playingWithWheel, system.config)):
...
def configureINI(config_directory: Path, bios_directory: Path, system: Emulator, rom: Path, controllers: Controllers, metadata: Mapping[str, str], guns: Guns, wheels: DeviceInfoMapping, playingWithWheel: bool) -> None:
    ...
    pcsx2INIConfig.set("Folders", "Savestates", "../../../saves/ps2/armsx2/sstates")
    pcsx2INIConfig.set("Folders", "MemoryCards", "../../../saves/ps2/armsx2")
    ...
    pcsx2INIConfig.set("Folders", "Videos", "../../../saves/ps2/armsx2/videos")
    ...
    # Fastboot
    pcsx2INIConfig.set("EmuCore", "EnableFastBoot", system.config.get_bool('armsx2_fastboot', True, return_values=("false", "true")))

    # Cheats
    pcsx2INIConfig.set("EmuCore", "EnableCheats", system.config.get('armsx2_cheats', "false"))

    # Widescreen Patches
    pcsx2INIConfig.set("EmuCore", "EnableWideScreenPatches", system.config.get("armsx2_EnableWideScreenPatches", "false"))

    # No-interlacing Patches
    pcsx2INIConfig.set("EmuCore", "EnableNoInterlacingPatches", system.config.get("armsx2_interlacing_patches", "false"))
    ...
    if vulkan.is_available():
        ...
        if gfxbackend := system.config.get("armsx2_gfxbackend"):
            if gfxbackend == "12":
    ...
    # Ratio
    pcsx2INIConfig.set("EmuCore/GS", "AspectRatio", system.config.get("armsx2_ratio", "Auto 4:3/3:2"))

    # Vsync
    pcsx2INIConfig.set("EmuCore/GS","VsyncEnable", system.config.get("armsx2_vsync", "0"))

    # Resolution
    pcsx2INIConfig.set("EmuCore/GS", "upscale_multiplier", system.config.get("armsx2_resolution", "1"))

    # FXAA
    pcsx2INIConfig.set("EmuCore/GS", "fxaa", system.config.get("armsx2_fxaa", "false"))

    # FMV Ratio
    pcsx2INIConfig.set("EmuCore/GS", "FMVAspectRatioSwitch", system.config.get("armsx2_fmv_ratio", "Auto 4:3/3:2"))

    # Mipmapping
    pcsx2INIConfig.set("EmuCore/GS", "mipmap_hw", system.config.get("armsx2_mipmapping", "-1"))

    # Trilinear Filtering
    pcsx2INIConfig.set("EmuCore/GS", "TriFilter", system.config.get("armsx2_trilinear_filtering", "-1"))

    # Anisotropic Filtering
    pcsx2INIConfig.set("EmuCore/GS", "MaxAnisotropy", system.config.get("armsx2_anisotropic_filtering", "0"))

    # Dithering
    pcsx2INIConfig.set("EmuCore/GS", "dithering_ps2", system.config.get("armsx2_dithering", "2"))

    # Texture Preloading
    pcsx2INIConfig.set("EmuCore/GS", "texture_preloading", system.config.get("armsx2_texture_loading", "2"))

    # Deinterlacing
    pcsx2INIConfig.set("EmuCore/GS", "deinterlace_mode", system.config.get("armsx2_deinterlacing", "0"))

    # Anti-Blur
    pcsx2INIConfig.set("EmuCore/GS", "pcrtc_antiblur", system.config.get("armsx2_blur", "true"))

    # Integer Scaling
    pcsx2INIConfig.set("EmuCore/GS", "IntegerScaling", system.config.get("armsx2_scaling", "false"))

    # Blending Accuracy
    pcsx2INIConfig.set("EmuCore/GS", "accurate_blending_unit", system.config.get("armsx2_blending", "1"))

    # Texture Filtering
    pcsx2INIConfig.set("EmuCore/GS", "filter", system.config.get("armsx2_texture_filtering", "2"))

    # Bilinear Filtering
    pcsx2INIConfig.set("EmuCore/GS", "linear_present_mode", system.config.get("armsx2_bilinear_filtering", "1"))

    # Load Texture Replacements
    pcsx2INIConfig.set("EmuCore/GS", "LoadTextureReplacements", system.config.get("armsx2_texture_replacements", "false"))

    # OSD messages
    osd_enabled = system.config.get("armsx2_osd_messages", "true")
    pcsx2INIConfig.set("EmuCore/GS", "OsdShowMessages", osd_enabled)

    # OSD Messages Position
    pcsx2INIConfig.set("EmuCore/GS", "OsdMessagesPos", "0" if osd_enabled == "false" else system.config.get("armsx2_osd_messages_position", "2"))

    # OSD Performance Position
    pcsx2INIConfig.set("EmuCore/GS", "OsdPerformancePos", system.config.get("armsx2_osd_performance_position", "0"))

    # Crop Overscan
    cropOverscan = "3" if system.config.get_bool("armsx2_overscan") else "0"
    ...
    # TV Shader
    pcsx2INIConfig.set("EmuCore", "TVShader", system.config.get("armsx2_shaderset", "0"))
    ...
    # VU thread speedhack
    pcsx2INIConfig.set("EmuCore/Speedhacks", "vuThread", system.config.get_bool("armsx2_vuthread", return_values=("true", "false")))

    # EE Cycle Rate speedhack
    pcsx2INIConfig.set("EmuCore/Speedhacks", "EECycleRate", system.config.get("armsx2_eecyclerate", "0"))
    ...
    if pcsx2INIConfig.has_section("USB1"):
        if system.config.get("armsx2_crosshairs") == "1":
            pcsx2INIConfig.set("USB1", "guncon2_cursor_path", str(_ARMSX2_RESOURCES_DIR / "crosshairs" / "default.png"))
    ...
    if pcsx2INIConfig.has_section("USB2"):
        if system.config.get("armsx2_crosshairs") == "1":
            pcsx2INIConfig.set("USB2", "guncon2_cursor_path", str(_ARMSX2_RESOURCES_DIR / "crosshairs" / "default.png"))
    ...
    fog_files = [
        _ARMSX2_RESOURCES_DIR / "textures" / "SCES-52530" / "replacements" / "c321d53987f3986d-eadd4df7c9d76527-00005dd4.png",
        _ARMSX2_RESOURCES_DIR / "textures" / "SLUS-20927" / "replacements" / "c321d53987f3986d-eadd4df7c9d76527-00005dd4.png"
    ]
    texture_dir = config_directory / "textures"
    # copy textures if necessary to PCSX2 config folder
    if system.config.get("armsx2_crisis_fog") == "true":
    ...
    # wheels
    wtype = Armsx2Generator.getWheelType(metadata, playingWithWheel, system.config)
    _logger.info("PS2 wheel type is %s", wtype)
    if Armsx2Generator.useEmulatorWheels(playingWithWheel, wtype) and wheels:
        wheelMapping = {
            "DrivingForcePro": {
                "up":       "Pad_DPadUp",
                "down":     "Pad_DPadDown",
        ...
        for pad in controllers:
            if pad.device_path in wheels:
                ...
                wheel_type = Armsx2Generator.getWheelType(metadata, playingWithWheel, system.config)
                pcsx2INIConfig.set(f"USB{usbx}", "Pad_subtype", Armsx2Generator.wheelTypeMapping[wheel_type])
    ...
    multitap_config = system.config.get("armsx2_multitap")

...
local function boot_graphmaths(cassette_handler)
    common_autoboot.type_at_frame(frame_num, "MON\n", 100)
	common_autoboot.type_at_frame(frame_num, "L\n", 200)
    common_autoboot.type_at_frame(frame_num, "GRM\n", 300)
...
local function boot_yellowcab(cassette_handler)
    -- common_autoboot.play_cassette_at_frame(frame_num, ":cassette", 100)
    common_autoboot.type_at_frame(frame_num, "MON\n", 100)
	common_autoboot.type_at_frame(frame_num, "L\n", 200)
    common_autoboot.type_at_frame(frame_num, "CAB\n", 300)
    common_autoboot.play_cassette_at_frame(frame_num, ":cassette", 400)
...
local boot_sequences = {
    graphmaths = boot_graphmaths,
    yellowcab = boot_yellowcab,
}

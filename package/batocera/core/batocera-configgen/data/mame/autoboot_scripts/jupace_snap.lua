...
local function boot_milliped() 
    common_autoboot.type_at_frame(frame_num, "milliped\n", 200)
end

local function boot_pacman() 
...
local current_software_name = manager.machine.images:at(SOFTWARE_SLOT).filename

-- Map software names to their respective boot functions
local boot_sequences = {
    ['default'] = boot_default,
    milliped = boot_milliped,
    pacman = boot_pacman,
}

...
define BATOCERA_PACMAN_INSTALL_CONF
	...
	$(INSTALL) -D -m 0644 $(BR2_EXTERNAL_BATOCERA_PATH)/package/batocera/utils/pacman/batocera_pacman.conf $(TARGET_DIR)/etc/batocera_pacman.conf
	$(INSTALL) -D -m 0644 $(BR2_EXTERNAL_BATOCERA_PATH)/package/batocera/utils/pacman/retrolx_pacman.conf $(TARGET_DIR)/etc/retrolx_pacman.conf
	...
	sed -i -e s+"{BATOCERA_ARCHITECTURE}"+"$(BATOCERA_SYSTEM_ARCH)"+ $(TARGET_DIR)/etc/batocera_pacman.conf
	sed -i -e s+"{BATOCERA_ARCHITECTURE}"+"$(BATOCERA_SYSTEM_ARCH)"+ $(TARGET_DIR)/etc/retrolx_pacman.conf

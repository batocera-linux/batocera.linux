...
class Daemon:
    ...
    async def __handle_device_event(self, event: evdev.InputEvent, action: str, begin: bool) -> None:
    ...
    def __cleanup_device_event_read_loop(self, path: str, /) -> None:
        self.mappings.pop(path, None)
        self.device_tasks.pop(path, None)

        if (device := self.input_devices.pop(path, None)) is not None:
            try:
                device.close()
            except OSError:
                pass

    async def __device_event_read_loop(self, device: evdev.InputDevice[str], /) -> None:
        path = device.path
        ...
        try:
            async for event in device.async_read_loop():
                ...
                if event.value == 1:
                    await self.__handle_device_event(event, action, True)
                elif event.value == 0:
                    await self.__handle_device_event(event, action, False)
            ...
            self.__cleanup_device_event_read_loop(path)

    async def __stop_device_read_loop(self, path: str, /) -> None:
        if (task := self.device_tasks.get(path)) is None:
            return
        ...
        # __device_event_read_loop(), so its finally block does not run.
        self.__cleanup_device_event_read_loop(path)
        ...
        try:
            if associations:
                ...
                self.device_tasks[path] = asyncio.create_task(
                    self.__device_event_read_loop(input_device),
                    name=f'evdev:{path}',
                )
                watched = True
        finally:
    ...
    async def __handle_udev_add(self, device: pyudev.Device, /) -> None:
        ...
        # If udev reports the same path again, completely retire the old
        # reader before opening the replacement.
        await self.__stop_device_read_loop(device.device_node)
        self.__add_udev_device(device)

    async def __handle_udev_remove(self, device: pyudev.Device, /) -> None:
        assert device.device_node is not None

        if self.debug and (input_device := self.input_devices.get(device.device_node)) is not None:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            await cancel_all(*self.background_tasks, *self.device_tasks.values(), self.udev_task)

...
NFC_WRITE_TAG = Path("/var/run/batocera-nfc-write-tag")
NFC_WRITE_TIME = 30
...
def callScripts(name, game):
    ...
    sysdirectory = Path("/usr/share/batocera/scripts/" + name)
    userdirectory = Path("/userdata/system/configs/emulationstation/scripts/" + name)
    directory = None
...
def on_connect(tag):
    ...
    if NFC_WRITE_TAG.exists() and (time.time() - NFC_WRITE_TAG.stat().st_mtime) < NFC_WRITE_TIME: # ignore the tag if is has more than Xs
        gwrite = True
        if not tag.ndef.is_writeable:
            print("write only tag.")
            return True
        record = ndef.TextRecord(NFC_WRITE_TAG.read_text(), "en")
        tag.ndef.records = [record]
        if gdebug:
            print("Written.")
        NFC_WRITE_TAG.unlink()
...
def nfc_write(txt):
    global gdebug
    NFC_WRITE_TAG.write_text(args.write)
    ntime = NFC_WRITE_TIME
    while NFC_WRITE_TAG.exists() and ntime > 0:
        if gdebug:
            print("waiting a tag (" + str(ntime) + ")")
        time.sleep(1)
        ntime = ntime -1
    if NFC_WRITE_TAG.exists():
        return False
    return True

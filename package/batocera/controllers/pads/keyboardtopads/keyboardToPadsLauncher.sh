...
# user keyboardsToPads configs are store in /boot, to get them working when plugged at boot.
...
test -e "/boot/configs/keyboardToPads/inputs/${CONFIGNAME}" && CONFIGFILE="/boot/configs/keyboardToPads/inputs/${CONFIGNAME}"
...
then
    ...
    if test -e "/boot/configs/keyboardToPads/inputs/${CONFIGNAME}"
    then
	echo 0
	exit 0
    fi
...
then
    ...
    echo "checking /boot/configs/keyboardToPads/inputs/${CONFIGNAME}..."

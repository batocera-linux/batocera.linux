...
define MAME_INSTALL_TARGET_CMDS
	# Create specific directories on target to store MAME distro
	mkdir -p $(MAME_PKG_DIR)/
	mkdir -p $(MAME_PKG_DIR)/ctrlr
	mkdir -p $(MAME_PKG_DIR)/docs/legal
	mkdir -p $(MAME_PKG_DIR)/docs/man
	mkdir -p $(MAME_PKG_DIR)/docs/swlist
	mkdir -p $(MAME_PKG_DIR)/hash
	mkdir -p $(MAME_PKG_DIR)/ini/examples
	mkdir -p $(MAME_PKG_DIR)/ini/presets
	mkdir -p $(MAME_PKG_DIR)/language
	mkdir -p $(MAME_PKG_DIR)/roms

	# Install binaries and default distro
    $(INSTALL) -D $(@D)/mamearcade	$(MAME_PKG_DIR)/mame
    $(INSTALL) -D $(@D)/mess		$(MAME_PKG_DIR)/mess
    cp $(@D)/COPYING				$(MAME_PKG_DIR)/
    cp $(@D)/README.md				$(MAME_PKG_DIR)/
    cp $(@D)/uismall.bdf			$(MAME_PKG_DIR)/
	cp -R $(@D)/artwork				$(MAME_PKG_DIR)/
	cp -R $(@D)/bgfx				$(MAME_PKG_DIR)/
	cp -R $(@D)/hash				$(MAME_PKG_DIR)/
	cp -R $(@D)/hlsl				$(MAME_PKG_DIR)/
	cp -R $(@D)/ini					$(MAME_PKG_DIR)/
	cp -R $(@D)/keymaps				$(MAME_PKG_DIR)/
    cp -R $(@D)/language			$(MAME_PKG_DIR)/
	cp -R $(@D)/plugins				$(MAME_PKG_DIR)/
	# Skip regression tests
	#cp -R $(@D)/regtests			$(MAME_PKG_DIR)/
	cp -R $(@D)/roms				$(MAME_PKG_DIR)/
	cp -R $(@D)/samples				$(MAME_PKG_DIR)/
	cp -R $(@D)/web					$(MAME_PKG_DIR)/

	# MAME tools
	$(INSTALL) -D $(@D)/castool		$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/chdman		$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/floptool	$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/imgtool		$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/jedutil		$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/ldresample	$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/ldverify	$(MAME_PKG_DIR)/
	$(INSTALL) -D $(@D)/romcmp		$(MAME_PKG_DIR)/

    # MAME dev tools skipped
	#$(INSTALL) -D $(@D)/unidasm	$(MAME_PKG_DIR)/
    #$(INSTALL) -D $(@D)/nltool		$(MAME_PKG_DIR)/
    #$(INSTALL) -D $(@D)/nlwav		$(MAME_PKG_DIR)/

	# Delete .po translation files
	# find $(TARGET_DIR)/usr/bin/mame/language -name "*.po" -type f -delete

	# Delete bgfx shaders for DX9/DX11/Metal
	rm -Rf $(MAME_PKG_DIR)/bgfx/shaders/metal/
	rm -Rf $(MAME_PKG_DIR)/bgfx/shaders/dx11/
	rm -Rf $(MAME_PKG_DIR)/bgfx/shaders/dx9/

	# Delete useless hash softlist files
	#rm -Rf $(MAME_PKG_DIR)/hash/vgmplay.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/amigaocs_flop.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/dc.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/gameboy.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/gba.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/gbcolor.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/megadriv.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/nes.hsi
	#rm -Rf $(MAME_PKG_DIR)/hash/nes.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/psx.xml
	#rm -Rf $(MAME_PKG_DIR)/hash/snes.xml

	# Evmapy mapper file
	cp $(BR2_EXTERNAL_BATOCERA_PATH)/package/retrolx/emulators/mame/mame.mame.keys $(MAME_PKG_DIR)/
